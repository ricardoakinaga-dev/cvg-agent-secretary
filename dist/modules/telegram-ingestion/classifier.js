...
class ClassifierService {
    ...
    classify(content, title) {
        ...
        let bestMatch = 'instruction'; // Default
        let highestScore = 0;
        const detectedTags = [];
     ...
     */
    extractTitle(content, providedTitle) {
        if (providedTitle) {
    ...
    validate(content) {
        ...
        if (content.length < types_1.CONTENT_LIMITS.minLength) {
            errors.push(`Content too short: minimum ${types_1.CONTENT_LIMITS.minLength} characters, got ${content.length}`);
        }
        // Check maximum length
        if (content.length > types_1.CONTENT_LIMITS.maxLength) {
            warnings.push(`Content too long: will be truncated to ${types_1.CONTENT_LIMITS.maxLength} characters`);
        }
...
exports.classifierService = new ClassifierService();
